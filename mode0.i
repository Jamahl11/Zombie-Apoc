...
int collisionClimb(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    current = collisionMap[((row + ((height - 1) / 2) + rowShift) * (mapWidth) + (col + ((width - 1) / 2) + colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }


    current = collisionMap[((row + rowShift) * (mapWidth) + (col + ((width - 1) / 2) + colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }


    current = collisionMap[((row + height - 1 + rowShift) * (mapWidth) + (col + ((width - 1) / 2) + colShift))];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }

...
int collisionClimb(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    // center middle
    current = collisionMap[OFFSET(col + ((width - 1) / 2) + colShift, row + ((height - 1) / 2) + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }

    // center Top
    current = collisionMap[OFFSET(col + ((width - 1) / 2) + colShift, row + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }

    //center bottom
    current = collisionMap[OFFSET(col + ((width - 1) / 2) + colShift, row + height - 1 + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
